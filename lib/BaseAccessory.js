class BaseAccessory {
    ...
    _registerTranslators(translators) {
        this.translators = { ...this.translators, ...translators };
        delete this.translators[undefined];
        delete this.translators[false];
        delete this.translators[true];
    }
    ...
    getTranslatedState(dp, callback) {
        this.getState(dp, (err, data) => {
            if (err) return callback(err);

            callback(null, this._getTranslatedState(dp, data));
        });
    }

    /* 0: translation of false, 0
         * 1: translation of true, 1
         * 2: transformer function, true for flipping
         */
    _getTranslatedState(dp, data) {
        let transformer = this.translators[dp][2],
            value;

        if (typeof transformer === 'function') value = transformer(data);
        else value = data ^ transformer;

        return [0, 1, false, true].includes(value) ? this.translators[dp][value] : data;
    }
